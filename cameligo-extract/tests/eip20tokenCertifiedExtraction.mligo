...
let init_amount (s : setup) = match s with 
Build_setup (owner, init_amount) -> init_amount

let owner (s : setup) = match s with 
Build_setup (owner, init_amount) -> owner

let balances (s : state) = match s with 
Build_state (total_supply, balances, allowances) -> balances

let increment_balance (m :  (address, tokenValue) map) (addr : address) (inc : tokenValue) = match Map.find_opt addr m with 
Some (old) -> (Map.add addr (addInt old inc) m)
 | None  -> (Map.add addr inc m)

let total_supply (s : state) = match s with 
Build_state (total_supply, balances, allowances) -> total_supply

let allowances (s : state) = match s with 
Build_state (total_supply, balances, allowances) -> allowances

let try_transfer (from : address) (to0 : address) (amount0 : tokenValue) (state : state) = let from_balance = match Map.find_opt from (balances state) with 
Some (v) -> v
 | None  -> 0 in 
...
let try_transfer_from (delegate : address) (from : address) (to0 : address) (amount0 : tokenValue) (state : state) = match Map.find_opt from (allowances state) with 
...
let try_approve (caller : address) (delegate : address) (amount0 : tokenValue) (state : state) = match Map.find_opt caller (allowances state) with 
...
let receive (ctx : cctx) (state : state) (maybe_msg :  (msg) option) = let sender0 = Tezos.sender in 
let without_actions = fun (o :  (state) option) -> match o with 
Some (a) -> (Some ( (a, ([]: (operation) list))))
 | None  -> (None: ((state *  (operation) list)) option) in 
if gtbTez Tezos.amount 0tez then (None: ((state *  (operation) list)) option) else match maybe_msg with 
Some (m) -> (match m with 
...
let receive_ (chain : chain) (ctx : cctx) (state : state) (maybe_msg :  (msg) option) = match receive ctx state maybe_msg with 
Some (x) -> (Some ( (x.1, x.0)))
