...
let set_delegate_call :  (operation) list = ([]: (operation) list)
...
let receive(chain : chain) (ctx : cctx) (state : dexter2CPMM_State) (maybe_msg :  (dEX2Extract_Msg) option) : dEX2Extract_result = match maybe_msg with 
Some (m) -> (match m with 
...
let receive_(chain : chain) (ctx : cctx) (state : dexter2CPMM_State) (maybe_msg :  (dEX2Extract_Msg) option) :  (( (operation) list * dexter2CPMM_State)) option = match receive chain ctx state maybe_msg with 
Some (x) -> (Some ( (x.1, x.0)))
