...
fn init(&'a self, chain: &'a Chain<'a>, ctx: &'a ContractCallContext<'a>, setup: ()) -> Result<Storage<'a>, Error<'a>> {
  Ok(
    self.alloc(
      List::nil(
        PhantomData)))
}
fn init__curried(&'a self) -> &'a dyn Fn(&'a Chain<'a>) -> &'a dyn Fn(&'a ContractCallContext<'a>) -> &'a dyn Fn(()) -> Result<Storage<'a>, Error<'a>> {
...
fn interp(&'a self, ext: Ext_map<'a>, insts: &'a List<'a, &'a Instruction<'a>>, s: Storage<'a>, cond: i64) -> Result<Storage<'a>, Error<'a>> {
  match insts {
    &List::nil(_) => {
      Ok(
        s)
...
fn interp__curried(&'a self) -> &'a dyn Fn(Ext_map<'a>) -> &'a dyn Fn(&'a List<'a, &'a Instruction<'a>>) -> &'a dyn Fn(Storage<'a>) -> &'a dyn Fn(i64) -> Result<Storage<'a>, Error<'a>> {
...
fn receive(&'a self, chain: &'a Chain<'a>, ctx: &'a ContractCallContext<'a>, s: Storage<'a>, msg: Option<Msg<'a>>) -> Result<__pair<Storage<'a>, &'a List<'a, ActionBody<'a>>>, Error<'a>> {
  match msg {
    Some(m) => {
      __pair_elim!(
        ext, insts, {
...
fn receive__curried(&'a self) -> &'a dyn Fn(&'a Chain<'a>) -> &'a dyn Fn(&'a ContractCallContext<'a>) -> &'a dyn Fn(Storage<'a>) -> &'a dyn Fn(Option<Msg<'a>>) -> Result<__pair<Storage<'a>, &'a List<'a, ActionBody<'a>>>, Error<'a>> {
  self.closure(move |chain| {
    self.closure(move |ctx| {
      self.closure(move |s| {
